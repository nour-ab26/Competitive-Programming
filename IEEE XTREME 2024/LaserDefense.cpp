...
int CountLower(const std::set<int>& set1, const std::set<int>& set2) {
    ...
    for (int num : set2) {
        // Use lower_bound to find the count of elements in set1 that are less than `num`
        auto it = set1.lower_bound(num);
        total += std::distance(set1.begin(), it); // Add the count to the total
    }
...
int main(){
    ...
    int l,n,m;
    cin>>l>>n>>m;
    int NaU=0,NbU=0,NaR=0,NbL=0,area=0;
    set<int> aU,bU;
    for (int i=0;i<n+m;i++){
        char c;
        int coor;
    ...
    area=NaU+NaR+1;
    area+=NbL*area;
    int intersect=CountLower(bU,aU);
    area+=(NbU)*NaR+1+intersect;
    cout<<area;
