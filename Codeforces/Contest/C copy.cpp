...
void solve() {
    ...
    while (t--) {
        ...
        if (unhappyCount > n / 2) {
            cout << 0 << endl; // No additional gold is needed
            continue;
        }

        // Calculate required additional gold
        double requiredGold = 0.0;
        while (true) {
            double averageAfterX = (totalWealth + requiredGold) / n;
            unhappyCount = 0;

            for (int i = 0; i < n; i++) {
                if (a[i] < (1.0 / 2.0) * averageAfterX) {
                    unhappyCount++;
                }
            }

            if (unhappyCount > n / 2) {
                cout << static_cast<long long>(requiredGold) << endl;
                break;
            }

            requiredGold += 1.0; // Increment the gold to test the next case
        }
    }
}
